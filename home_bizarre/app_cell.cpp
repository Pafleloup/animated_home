...
AppCell::AppCell() :
  HighlightCell(),
  m_nameView(KDFont::SmallFont, (I18n::Message)0, 0.5f, 0.5f, Palette::HomeCellText, KDColor::RGB24(0x667667)),
...
void AppCell::drawRect(KDContext * ctx, KDRect rect) const {
  ...
  //m_backgroundView->BackgroundInnerView::drawRect(ctx, KDRect(0, 0,320, 240));
  //m_backgroundView->drawRect( KDRect(0, 0,320, 240));
...
void AppCell::reloadCell() {
  if(isHighlighted()){
    m_nameView.setFont(KDFont::LargeFont);
  }else{
    m_nameView.setFont(KDFont::SmallFont);
  }
  layoutSubviews(1);
  ...
  m_nameView.setBackgroundColor(isHighlighted() ? KDColor::RGB24(0x911911): KDColor::RGB24(0x667667));
